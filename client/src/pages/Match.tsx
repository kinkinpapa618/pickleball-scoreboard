...
import { RotateCcw, Trophy, CheckCircle2, AlertOctagon, Undo2, Home } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="px-4 py-3 border-b flex items-center justify-between bg-white/80 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="font-display font-bold text-lg text-primary">BMB Pickleball Scoreboard</div>
        <Button variant="ghost" size="sm" onClick={undo} disabled={state.gameHistory.length === 0} className="gap-2">
      ...
      <Dialog open={!!state.winner}>
        <DialogContent className="sm:max-w-md text-center border-none shadow-none bg-transparent">
          <div className="bg-white/95 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-white/40">
            ...
            <DialogTitle className="text-4xl font-black text-foreground mb-2">CHIẾN THẮNG!</DialogTitle>
            <p className="text-2xl text-primary font-bold mb-8">
              ĐỘI {state.winner === 1 ? "1" : "2"}
            </p>
            
            <div className="text-lg text-muted-foreground mb-8">
              Tỉ số: <span className="font-bold text-foreground">{state.score1} - {state.score2}</span>
            </div>

            <div className="flex flex-col gap-3">
              <Button size="lg" className="w-full font-bold text-lg h-14" onClick={() => window.location.reload()}>
                <RotateCcw className="mr-2 w-5 h-5" />
                Chơi lại
              </Button>
              <Button variant="outline" size="lg" className="w-full font-bold h-14" onClick={() => setLocation("/")}>
                Về trang chủ
              </Button>
            </div>
