...
import { useCreateMatch, useUpdateMatch } from "@/hooks/use-api"; // Import thêm useUpdateMatch
...
export default function Match() {
  ...
  // Lấy ID trận đấu nếu đang trong luồng cập nhật trận đấu cũ
  ...
  const updateMatch = useUpdateMatch(); // Hook để đẩy dữ liệu lên Livestream
  const [saved, setSaved] = useState(false);

  const isFirstServeActive = (state as any).firstServe || (state as any).isFirstServe;

  // HÀM ĐỒNG BỘ: Đẩy dữ liệu lên server mỗi khi trạng thái trận đấu thay đổi
  useEffect(() => {
    const syncData = getMatchData();
    if (matchId && syncData) {
      updateMatch.mutate({
        id: parseInt(matchId),
        data: {
          ...syncData,
          // Thêm các trường cho Livestream
  ...
  }, [state.score1, state.score2, state.serverTeam, state.serverHand]);
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex flex-col font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col p-4 space-y-4 max-w-3xl mx-auto w-full">
        ...
        <div className="grid grid-cols-2 gap-4 pb-4">
          <motion.button
            ...
            className="group relative h-24 rounded-3xl bg-[#ccff00] overflow-hidden shadow-[0_20px_40px_rgba(204,255,0,0.15)] active:brightness-90 transition-all"
          >
            <div className="absolute inset-0 bg-gradient-to-tr from-black/20 to-transparent" />
            <div className="relative flex flex-col items-center justify-center text-black">
              <CheckCircle2 className="w-8 h-8 mb-1" />
              <span className="text-[10px] font-black italic uppercase tracking-[0.2em]">Ghi Điểm</span>
            </div>
          </motion.button>
          ...
          <motion.button
            ...
            onClick={fault} 
            disabled={!!state.winner}
            className="group relative h-24 rounded-3xl bg-slate-800 border-2 border-white/5 overflow-hidden active:bg-slate-700 transition-all"
          >
            <div className="relative flex flex-col items-center justify-center text-white">
              <AlertOctagon className="w-8 h-8 mb-1 text-rose-500" />
              <span className="text-[10px] font-black italic uppercase tracking-[0.2em] text-white/40">Lỗi/Đổi Giao</span>
            </div>
          </motion.button>
        </div>
      ...
      <Dialog open={!!state.winner}>
        <DialogContent className="max-w-xs bg-slate-900 border-white/10 rounded-[2rem] p-8 outline-none">
          <div className="text-center">
            ...
            <div className="space-y-3">
              ...
              <Button variant="ghost" onClick={() => setLocation("/")} className="w-full text-white/40 font-bold h-12 hover:text-white">
                MENU CHÍNH
              </Button>
            </div>
          </div>
