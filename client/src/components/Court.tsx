...
export function Court({
  ...
  const getPlayerInfo = (team: number, player: number) => {
    ...
    // Xác định vị trí hiển thị dựa trên team và position
    let leftPosition = "";
    let topPosition = "";

    if (team === 1) {
      // Đội 1: bên trái sân
      if (position === "right") { // Ô 1
        leftPosition = "left-[25%]";
        topPosition = "top-[75%]"; // Dưới
      } else { // position === "left" - Ô 2
        leftPosition = "left-[25%]";
        topPosition = "top-[25%]"; // Trên
      }
    } else { // team === 2
      // Đội 2: bên phải sân
      if (position === "right") { // Ô 1
        leftPosition = "left-[75%]";
        topPosition = "top-[25%]"; // Trên
      } else { // position === "left" - Ô 2
        leftPosition = "left-[75%]";
        topPosition = "top-[75%]"; // Dưới
      ...
      {players.map(player => (
        <div 
          ...
          className={`
            ...
            ${player.leftPosition} ${player.topPosition}
