...
export default function TournamentManager() {
  ...
  const [players] = useState<string[]>(["Minh", "Hoàng", "Tuấn", "Dũng", "Sơn", "Hải", "Lâm", "Đạt"]);
  ...
  const handleExport = () => {
    ...
    const data = { stats, history, matches, exportDate: new Date().toLocaleString() };
    ...
    <div className="space-y-8 pb-24">
      ...
      <section className="bg-rose-500/5 border border-rose-500/10 rounded-[2rem] p-6 flex flex-col md:flex-row items-center justify-between gap-4 shadow-[0_0_30px_rgba(244,63,94,0.05)]">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-white/30 text-[9px] font-bold uppercase tracking-widest">Control Center</p>
          </div>
        </div>
      ...
      <section>
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="text-white font-black italic text-xl uppercase tracking-tighter">Thiết lập giải đấu</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { id: "ELIMINATION", title: "Loại trực tiếp", icon: <GitBranch />, desc: "Knockout Stage" },
            { id: "ROUND_ROBIN", title: "Vòng tròn", icon: <RotateCw />, desc: "League Format" },
            { id: "GROUP_STAGE", title: "Chia bảng A-B", icon: <Layers />, desc: "Group Qualifiers" }
          ].map((item) => (
            <button
              key={item.id}
          ...
          <AnimatePresence>
            {matches.length > 0 ? (
                    ...
                    <CardContent className="p-5">
                      ...
                      <div className="flex justify-between items-start mb-4 border-b border-white/5 pb-3">
                        ...
                        <div className="relative group/select">
                          <select
                            ...
                            `}
                          >
                            <option value="0" className="bg-slate-900 text-white">CHỜ SÂN...</option>
                            {AVAILABLE_COURTS.map(c => (
                              <option key={c} value={c} className="bg-slate-900 text-white">SÂN {c}</option>
                            ))}
                          </select>
                          {/* Mũi tên trang trí */}
                          <div className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none">
                             <div className={`w-0 h-0 border-l-[3px] border-l-transparent border-r-[3px] border-r-transparent border-t-[4px] ${m.status === 'playing' ? 'border-t-black' : 'border-t-white/40'}`} />
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4 relative z-10">
                        ...
                        <div className="flex justify-between items-center">
                          <span className={`font-black italic uppercase text-sm tracking-tight transition-colors ${m.status === 'playing' ? 'text-white text-lg' : 'text-white/70'}`}>
                            {m.p1}
                          </span>
                          {/* Chấm xanh nhấp nháy */}
                          {m.status === "playing" && (
                        ...
                        <div className="relative h-px bg-white/5 w-full flex items-center justify-center">
                          <div className={`absolute px-2 text-[10px] font-black italic transition-colors uppercase ${
                            m.status === 'playing' 
              ...
              >
                ...
                <p className="text-white/20 font-black italic uppercase text-sm tracking-widest">Danh sách trống</p>
                <p className="text-white/10 text-[10px] uppercase mt-2">Vui lòng chọn thể thức và bấm Generate</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
